...
```bash
adb shell ping -c 3 10.20.128.51
```
...
MEDIASOUP_ANNOUNCED_IPS=10.20.128.51 pnpm start:dev
```
...
```bash
# Send transport
sudo iptables -I INPUT  -i wlan0 -p udp -d 172.30.1.78 --dport <X> -j DROP
sudo iptables -I OUTPUT -o wlan0 -p udp -s 172.30.1.78 --sport <X> -j DROP

# Recv transport
sudo iptables -I INPUT  -i wlan0 -p udp -d 172.30.1.78 --dport <Y> -j DROP
sudo iptables -I OUTPUT -o wlan0 -p udp -s 172.30.1.78 --sport <Y> -j DROP
```
